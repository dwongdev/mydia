  ...
  group('MediaTokenService', () {
    ...
    group('setToken', () {
      test('stores token and expiry time', () async {
        ...
        verify(mockStorage.write('auth_token', 'new-token')).called(1);
        verify(mockStorage.write('auth_token_expiry', expiresAt.toIso8601String())).called(1);
      });
    });
    ...
    group('needsRefresh', () {
      ...
      test('returns true when expiry time is not stored', () async {
        when(mockStorage.read('auth_token'))
            .thenAnswer((_) async => 'test-token');
        when(mockStorage.read('auth_token_expiry'))
            .thenAnswer((_) async => null);
    ...
    group('buildMediaUrl', () {
      ...
      test('appends token as query parameter', () async {
        ...
        expect(url, equals('https://example.com/api/v1/stream/file/123?token=test-token'));
      });
      ...
      test('uses ampersand when URL already has query parameters', () async {
        ...
        expect(url, equals('https://example.com/api/v1/stream/file/123?strategy=HLS&token=test-token'));
      });
    });
