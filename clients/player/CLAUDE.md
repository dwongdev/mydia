...
```bash
./dev up -d               # Start everything (Phoenix + Flutter with hot reload)
...
**Live reload is automatic!** Just run `./dev up -d`, open `http://localhost:4000/player`, and edit files. Changes trigger a rebuild (~40s) and the browser auto-refreshes.
